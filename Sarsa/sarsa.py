...
class Sarsa:
    def __init__(self, actions, EPOCHES, discount=0.9, learning_rate=0.1, greedy=0.9,
    ...
    def rl(self, env):
        ...
        for epoch in range(self.EPOCHES):
            ...
            for iteration in range(self.max_iteration):
                ...
                self.Q_table.loc[observation, action] = (1 - self.lr) * \
                                                        self.Q_table.loc[observation, action] + \
                                                        self.lr * \
                                                        (reward + self.discount * self.Q_table.loc[new_state, _action])
                if done:
                    if reward == 1:
                        x.append(epoch)
                        y.append(count)
...
if __name__ == '__main__':
    ...
    test = Sarsa(env.action_space, 100, fresh_time=0.1)
    x, y = test.rl(env)
    test.output_Q_table()
    pic = dv.LineChart()
